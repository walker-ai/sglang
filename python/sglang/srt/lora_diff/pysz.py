...
class SZ:
    ...
    def _get_thread_local_sz(self):
        """为每个线程获取独立的SZ实例"""
        if not hasattr(self._thread_local, 'sz'):
            self._thread_local.sz = self.sz  # 假设self.sz是线程安全的
        return self._thread_local.sz
    
    def _decompress_single(self, args):
        """单个解压缩任务"""
        data_cmpr, original_shape, original_dtype = args
        
        # 获取线程本地的SZ实例
        sz = self._get_thread_local_sz()
        
        # 准备参数
        r5, r4, r3, r2, r1 = [0] * (5 - len(original_shape)) + list(original_shape)
        ori_type, _ = self.__sz_datatype(original_dtype)
        
        # 设置返回类型
        sz.SZ_decompress.restype = ctypes.POINTER(
            ctypes.c_float if original_dtype == np.float32 else ctypes.c_double)
        
        # 执行解压缩
        data_dec_c = sz.SZ_decompress(
            ori_type,
        ...
        # 转换结果
        data_dec = np.array(
            data_dec_c[:np.prod(original_shape)], 
    ...
    def batch_decompress(self, data_list, shapes, dtypes=None):
        """
        批量解压缩
        ...
        Args:
            data_list: 压缩数据列表
            shapes: 原始形状列表
            dtypes: 数据类型列表，默认都是np.float32
        """
        if dtypes is None:
            dtypes = [np.float32] * len(data_list)
            
        # 准备任务列表
        tasks = list(zip(data_list, shapes, dtypes))
        
        # 使用线程池并行处理
        with ThreadPoolExecutor(max_workers=32) as executor:
            results = list(executor.map(self._decompress_single, tasks))
